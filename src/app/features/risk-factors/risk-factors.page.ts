          ...
          <section class="panel overflow-hidden">
            <div class="border-b-[1.5px] border-line px-4 py-3.5">
              <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h2 class="m-0 text-[15px] font-bold text-ink">
                    รายการโครงการเรียงตาม Risk Score
                  </h2>
                  <p class="m-0 mt-1 text-[12.5px] text-muted">
                    คลิกโครงการเพื่อดูรายละเอียดและปัจจัยที่ทำให้เสี่ยง
                  </p>
                </div>
                ...
                @for (project of filteredProjects(); track project.project_id) {
                  ...
                  >
                    ...
                    <p class="m-0 mt-1.5 text-xs text-muted">
                      ...
                      {{ money(project.budget_amount) }} บาท
                    </p>
                  </button>
                }
              </div>
              ...
              @if (loadingDetail()) {
                ...
                <section class="panel p-[18px]">
                  ...
                  @if (!triggeredFactors().length) {
                    <app-empty-state
                      title="ไม่พบ factor ที่ trigger"
                      message="โครงการนี้อาจไม่มีสัญญาณตามเกณฑ์ที่กำหนดไว้"
                    />
                  } @else {
                      ...
                      @for (factor of triggeredFactors(); track factor.factor_code) {
                        <article class="rounded-[4px] border-[1.5px] border-line p-3.5">
                          ...
                          @if (factor.legal_ref) {
                            ...
                            >
                              <span class="font-bold text-slate-600">ฐานอ้างอิง:</span>
                              {{ factor.legal_ref }}
                            </p>
                          }
                  ...
                  @if (notComputableFactors().length) {
                    <div class="mt-3.5 rounded-[3px] border border-[#e6cfca] bg-[#fdf6f5] p-3">
                      <p class="m-0 text-[12px] font-bold text-[#8a2a1f]">
                        ปัจจัยที่ประเมินไม่ได้ (ข้อมูลไม่เพียงพอ — ไม่ใช่ "ผ่าน")
                      </p>
                ...
                <section class="panel p-[18px]">
                  <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p class="m-0 mt-1 text-[13px] text-muted">
                        รายการ factor ทั้งหมดในระบบ เพื่อใช้อ้างอิงประกอบการตรวจสอบ
                      </p>
                    </div>
                    <span
                  ...
                  @if (!selectedProjectCatalog().length) {
                    <div class="mt-3">
                      <app-empty-state
                        title="ยังไม่มีข้อมูล catalog"
                        message="รอให้ backend ส่งรายการ risk factor"
                      />
                    </div>
                  } @else {
...
export class RiskFactorsPageComponent implements OnInit {
   ...
   * backend มาสก์ TIN ให้ public_user อยู่แล้ว (mask_project_for_public ใน privacy.py) — field นี้จึง
   * เป็นเลขที่ถูกมาสก์แล้ว (เช่น *********0123) แสดงได้ตรงๆ ไม่ต้องซ่อนทั้งหมดอีกชั้น */
